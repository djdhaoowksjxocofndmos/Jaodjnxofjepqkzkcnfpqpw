script_name("Sion Detek Pohon CPRP Siap Ga Siap Harus Siap")
script_author("Sion")
    ...
    for id = 0,2048 do
        ...
        if sampIs3dTextDefined(id) then
            ...
            if ok and text then
                ...
                if clean:lower():find("tree",1,true) then
...
local title = " SION DETEK POHON CPRP SIAP GA SIAP HARUS SIAP"
...
imgui.Begin(title, window, imgui.WindowFlags.AlwaysAutoResize+ imgui.WindowFlags.NoScrollbar)
    ...
    if imgui.Button("SCAN POHON", imgui.ImVec2(-1,40)) then
        scanTrees()
    end

 imgui.Separator()

local columnCount = math.ceil(#trees / 5)

for row = 1, 5 do

    for col = 1, columnCount do

        local index = row + ((col - 1) * 5)
...
local name = "Pohon Ga Siap"
local timer = string.format("%02d:%02d", menit, detik)

local space = math.floor((#name - #timer) / 2)

if space < 0 then
    space = 0
end

text = name.."\n"..string.rep(" ", space)..timer
...
if imgui.Button(text.."##"..index, imgui.ImVec2(size.x + 30, size.y + 20)) then
    selectedTree = v
end
...
sampRegisterChatCommand("sdph", function() window[0] = not window[0] end)

lua_thread.create(function()
    wait(5000)
    sampAddChatMessage("{39C0FF}[Sion Detek Pohon CPRP Siap Ga Siap Harus Siap] {FFFFFF}Loaded, ketik {39C0FF}/sdph {FFFFFF}buat buka UI.", -1)
end)
